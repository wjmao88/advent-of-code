...
const getMoveDest = (x, y, dir) => {
  switch(dir) {
    case 1: return [x, y + 1];
    case 2: return [x, y - 1];
    case 3: return [x - 1, y];
    case 4: return [x + 1, y];
    default: throw 'no move ' + currentDir;
  }
}
...
const createMapping = () => {
  ...
  while(hitOrigin < 4) {
    ...
    if (done) {
      ...
      lastHitWall = value === 0;
      if (lastHitWall) {
        const [ wx, wy ] = getMoveDest(x, y, currentDir);
        tileMap[wy] = tileMap[wy] || {};
        tileMap[wy][wx] = value;
      } else {
        [ x, y ] = getMoveDest(x, y, currentDir);
        tileMap[y] = tileMap[y] || {};
        tileMap[y][x] = value;
      }
      if (x === 0 && y === 0) {
...
const printDist = (dist) => {
  ...
  return dist.toString().padStart(3,' ');
};

const distMapFromOxygen = (tileMap) => {
  const oxygenY = findKey(tileMap, a => findKey(a, t => t === 2));
  const oxygenX = findKey(tileMap[oxygenY], t => t === 2);
  console.log('oxygen at', oxygenX, oxygenY)
  const distMap = { [oxygenY]: { [oxygenX]: 0 }};

  let bfs = [[+oxygenX, +oxygenY, 0]];
  while (bfs.length) {
...
const oxygenBot = () => {
  const tileMap = createMapping();
  console.log(printMap(tileMap, printTile));

  const distMap = distMapFromOxygen(tileMap);
  console.log('dist to oxygen is ', distMap[0][0]);

  console.log(printMap(distMap, printDist));
  const maxReach = Math.max(...flatMap(distMap, values));
  console.log('max reach time is ', maxReach)
}

console.log(oxygenBot())